...
Antigravity 另有**週額度**（`agy` 互動模式的 `/usage` 可以看剩多少）。週額度用完時，額度錯誤的 `Resets in` 會是幾十個小時，超過 6 小時的倒數不採用，改為指數退避，最長每 60 分鐘重試一次。監視程式看到這種暫停，會發「Antigravity 週額度用完」警報，同一類 6 小時內只送一次，提醒切換到另一個 Gemini 帳號（在 `agy` 互動模式 `/logout` 後重新登入）。換好帳號後，校正程式最慢一小時內會自動接上；要立刻接上，可以按 Ctrl-C 後用同樣的指令重跑。

...
- 因為預算緊，有三個原則：(1) 大量的 LLM 校正用訂閱額度（Antigravity 的 Gemini），不走付費 API；(2) GPU VM 不用就刪掉，停機時 200GB 磁碟每天約 21 台幣；(3) 使用者要求 GCS 上保留一份原始資料（2026-09-25），所以 `raw/` 不刪（原本打算第二步後刪掉），改成較便宜的儲存等級：封存（Archive）每月約 3 台幣，標準約 46 台幣；至少要存 365 天，全部讀回約 115 台幣。轉換要先問使用者；音訊第三步後再決定。

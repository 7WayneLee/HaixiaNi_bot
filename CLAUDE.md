...
- [ ] 第二步：抽音訊、轉錄
  ...
  - [x] 全量轉錄（2026-09-25 約 17:55 至 09-26 10:14，Whisper large-v3 不加提示詞，L4 一般計費）：
    - 440/440，0 個失敗，逐字稿在 `gs://haixiani-bot-data-507014/transcripts/asr/`（440 個 JSON，已核對）。
    - 監視用的 luna worker 在停機前因 Codex 額度用完而中斷，由指揮核對後在 11:05 停機。GPU VM 已停機，未刪除。
- [ ] 第三步：校對、切段、建索引
  ...
  - [ ] 全量校正（2026-09-26 06:37 開始，先跑已轉好的 310 檔；轉錄完成後再補其餘檔案）
    ...
    - 帳號接力（2026-09-26，使用者要求每次 5 小時額度用完也要提醒；程式由 Claude Code worker 撰寫，258 個測試通過）：
      - 校正程式用 `--quota-poll-min 10` 執行：額度暫停最多 10 分鐘，之後先派一個 worker 試探，成功才全部恢復。
